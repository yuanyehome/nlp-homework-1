...
if __name__ == '__main__':
    ...
    if args.train: 
        ...
        for i in range(args.iteration):
            print('第 %i 次迭代'%(i+1))
            evaluator=Evaluator()
            for l in tqdm(open(args.train).readlines(), desc="Training"):
            ...
            if args.dev :
                evaluator=Evaluator()
                for l in tqdm(open(args.dev).readlines(), desc="Validating"):
                    x,y=load_example(l.split())
                    z=cws.decode(x)
                    evaluator(dump_example(x,y),dump_example(x,z))
                evaluator.report()
    ...
    if args.test : 
        ...
        evaluator=Evaluator()
        for l in tqdm(open(args.test).readlines(), desc="Testing") :
            x,y=load_example(l.split())
            z=cws.decode(x)
            evaluator(dump_example(x,y),dump_example(x,z))
        evaluator.report()
    ...
    if args.model and (not args.train and not args.test) : 
        cws=CWS()
        cws.weights.load(args.model)
        instream=open(args.predict).readlines() if args.predict else sys.stdin
        outstream=open(args.result,'w') if args.result else sys.stdout
        for l in tqdm(instream):
            x,y=load_example(l.split())
            z=cws.decode(x)
